...
# This script is the CI (confidence interval) generator for pifcalc_main.R -- for the main (semi-supervised-learning) version of our algorithm. 
...
# See pifcalc_main.R for the parameter description.
...
PIFs_semi_super_2sets_boot3 <- function(dset_predict,dset_labeled,exper_d,path_d,neut_d,rel_toler,meth) {
  # See the function with a similar name in pifcalc_main.R
...
PIF_boot <- function(data,rel_tol,num_reps,analysis_var) {
  # This function performs bootstrap
...
quant_intervals <- function(boot_output) {
  # This function uses bootstrap results to calculate CIs
...
file_out <- paste("pifcalc_main_CIs_", as.integer(analysis_var), sep = "")
